const PROJECT_PREFIX = 'rum_project_';
...
router.get('/api/projects/list', async ({ request, user }) => {
    try {
        ...
        const projects = (await userPuter.kv.list(PROJECT_PREFIX, true))
            .map(({value}) => ({ ...value, isPublic: value.isPublic ?? value.visibility === 'public' }))
            .filter(p => p.ownerId === userId || p.isPublic);
...
router.get('/api/projects/get', async ({ request, user }) => {
    try {
        ...
        const isPublic = project.isPublic ?? project.visibility === 'public';
        if (project.ownerId !== userId && !isPublic) {
